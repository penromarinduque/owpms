                ...
                <div class="collapse {{ request()->routeIs('ltpapplication.index') ? 'show' : ''}}" id="adminApplicationsCollapse" aria-labelledby="headingOne" data-bs-parent="#sidenavAccordion">
                    <nav class="sb-sidenav-menu-nested nav">
                        ...
                        @if ($canViewSubmittedTab)
                            <a class="nav-link {{ request()->routeIs('ltpapplication.index') && request()->query('category') == 'submitted' ? 'active' : '' }}" href="{{ route('ltpapplication.index', ['category' => 'submitted', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('submitted') }}</span> Submitted</a> 
                        @endif
                        @if ($canViewReviewedTab)
                            <a class="nav-link {{ request()->routeIs('ltpapplication.index') && request()->query('category') == 'reviewed' ? 'active' : '' }}" href="{{ route('ltpapplication.index', ['category' => 'reviewed', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('reviewed') }}</span> Reviewed</a> 
                        @endif
                        <a class="nav-link {{ request()->routeIs('ltpapplication.index') && request()->query('category') == 'returned' ? 'active' : '' }}" href="{{ route('ltpapplication.index', ['category' => 'returned', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('returned') }}</span> Returned</a> 
                        @if ($canViewAcceptedTab)
                            <a class="nav-link {{ request()->routeIs('ltpapplication.index') && request()->query('category') == 'accepted' ? 'active' : '' }}" href="{{ route('ltpapplication.index', ['category' => 'accepted', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('accepted') }}</span> Accepted</a> 
                        @endif
                        <a class="nav-link {{ request()->routeIs('ltpapplication.index') && request()->query('category') == 'rejected' ? 'active' : '' }}" href="{{ route('ltpapplication.index', ['category' => 'rejected', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('rejected') }}</span> Rejected</a> 
                        @if ($canViewApprovedTab)
                            <a class="nav-link {{ request()->routeIs('ltpapplication.index') && request()->query('category') == 'approved' ? 'active' : '' }}" href="{{ route('ltpapplication.index', ['category' => 'approved', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('approved') }}</span> Approved</a> 
                        @endif
                        <a class="nav-link {{ request()->routeIs('ltpapplication.index') && request()->query('category') == 'expired' ? 'active' : '' }}" href="{{ route('ltpapplication.index', ['category' => 'expired', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('expired') }}</span> Expired</a> 
                    </nav>
                </div>
            @endif
            ...
            @if(Auth::user()->usertype=='permittee')
                ...
                <div class="collapse {{ request()->routeIs('myapplication.index') ? 'show' : ''}}" id="collapseLayouts" aria-labelledby="headingOne" data-bs-parent="#sidenavAccordion">
                    <nav class="sb-sidenav-menu-nested nav">
                        ...
                        <a class="nav-link {{ request()->routeIs('myapplication.index') && request()->query('category') == 'draft' ? 'active' : '' }}" href="{{ route('myapplication.index', ['category' => 'draft', 'status' => 'draft']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('draft') }}</span> Drafts</a> 
                        <a class="nav-link {{ request()->routeIs('myapplication.index') && request()->query('category') == 'submitted' ? 'active' : '' }}" href="{{ route('myapplication.index', ['category' => 'submitted', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('submitted') }}</span> Submitted</a> 
                        <a class="nav-link {{ request()->routeIs('myapplication.index') && request()->query('category') == 'reviewed' ? 'active' : '' }}" href="{{ route('myapplication.index', ['category' => 'reviewed', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('reviewed') }}</span> Reviewed</a> 
                        <a class="nav-link {{ request()->routeIs('myapplication.index') && request()->query('category') == 'returned' ? 'active' : '' }}" href="{{ route('myapplication.index', ['category' => 'returned', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('returned') }}</span> Returned</a> 
                        <a class="nav-link {{ request()->routeIs('myapplication.index') && request()->query('category') == 'accepted' ? 'active' : '' }}" href="{{ route('myapplication.index', ['category' => 'accepted', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('accepted') }}</span> Accepted</a> 
                        <a class="nav-link {{ request()->routeIs('myapplication.index') && request()->query('category') == 'rejected' ? 'active' : '' }}" href="{{ route('myapplication.index', ['category' => 'rejected', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('rejected') }}</span> Rejected</a> 
                        <a class="nav-link {{ request()->routeIs('myapplication.index') && request()->query('category') == 'approved' ? 'active' : '' }}" href="{{ route('myapplication.index', ['category' => 'approved', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('approved') }}</span> Approved</a> 
                        <a class="nav-link {{ request()->routeIs('myapplication.index') && request()->query('category') == 'expired' ? 'active' : '' }}" href="{{ route('myapplication.index', ['category' => 'expired', 'status' => 'all']) }}"><span class="badge rounded-pill bg-primary me-2">{{ $_helper->ltpApplicationCountByStatus('expired') }}</span> Expired</a> 
                    </nav>
                </div>
            @endif
            ...
            <div class="collapse @yield('for-signatures')" id="collapseForSignatures" aria-labelledby="headingOne" data-bs-parent="#sidenavAccordion">
                <nav class="sb-sidenav-menu-nested nav">
                    ...
                    @if (Auth::user()->usertype=='admin' || Auth::user()->usertype=='internal')
                        ...
                        <a class="nav-link" href="{{ route('for-signatures.index', ['type' => 'billing_statement']) }}">Assesment of Fees and Charges&nbsp;&nbsp;{!! $_helper->displayBadgeCount('primary', $_helper->getForSignatoriesCount('billing_statement')) !!}</a>

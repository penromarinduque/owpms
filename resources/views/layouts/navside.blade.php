            ...
            @if(Auth::user()->usertype=='permittee')
                ...
                <div class="collapse {{ request()->routeIs('myapplication.index') ? 'show' : ''}}" id="collapseLayouts" aria-labelledby="headingOne" data-bs-parent="#sidenavAccordion">
                    <nav class="sb-sidenav-menu-nested nav">
                        ...
                        <a class="nav-link {{ request()->routeIs('myapplication.index') && request()->query('category') == 'draft' ? 'active' : '' }}" href="{{ route('myapplication.index', ['category' => 'draft', 'status' => 'draft']) }}"><span class="badge rounded-pill bg-primary me-2">0</span> Drafts</a> 

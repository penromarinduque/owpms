...
<body class="p-4 font-monospace">
    ...
    <div class="row justify-content-end mb-3">
        <div class="col-6 text-end">
            <div class="mb-2">
                <span class="fw-bold">Bill No. <ul>{{ $paymentOrder->order_number }}</ul></span>
            </div>
            <div class="mb-2">
                <span class="fw-bold">Date: <ul>{{ $paymentOrder->created_at->format('F d, Y') }}</ul></span>
    ...
    <div class="mb-3">
        <label class="fw-bold mb-1">Name/Payee: {{ $paymentOrder->ltpApplication->permittee->user->personalInfo->first_name . ' ' . $paymentOrder->ltpApplication->permittee->user->personalInfo->last_name }}</label>
        <div class=" border-dark"></div>
    </div>

    <div class="mb-3">
        <label class="fw-bold mb-1">Address: {{ $paymentOrder->ltpApplication->permittee->user->wfp()->wildlifeFarm->barangay->barangay_name }}, {{ $paymentOrder->ltpApplication->permittee->user->wfp()->wildlifeFarm->barangay->municipality->municipality_name }}, {{ $paymentOrder->ltpApplication->permittee->user->wfp()->wildlifeFarm->barangay->municipality->province->province_name }}</label>
        <div class=" border-dark"></div>
    </div>

    <div class="mb-3">
        <label class="fw-bold mb-1">Nature of Application/Permit/Documents being secured:</label>
        <div class=" border-dark">Local Transport Permit</div>
    </div>

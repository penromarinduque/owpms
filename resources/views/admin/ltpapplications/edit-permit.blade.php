...
<div class="container-fluid px-4">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form class="row bg-light py-4 align-content-center justify-content-center" method="POST" action="{{ route('ltpapplication.updatePermit', Crypt::encryptString($ltp_application->id)) }}" enctype="multipart/form-data">
                @csrf
                <div class="col-8 bg-white p-5 mb-4">
                    ...
                    <p>
                        ...
                        <input style="width: 150px;" type="date" name="transport_date" value="{{ old('transport_date', $ltp_application->transport_date->format('Y-m-d')) }}" class="form-control d-inline">
                    ...
                    <div class="row">
                        {{-- <div class="col-sm-6">
                            <select name="chief_rps" id="chief_rps" class="form-select select2 @error('chief_rps') is-invalid @enderror">
                                ...
                                @foreach ($_user->getAllInternals() as $user)
                                    <option value="{{ $user->id }}" {{ $user->id == old('chief_rps') ? 'selected' : ($user->id == $permit->chief_rps ? 'selected' : '') }} style="text-transform: uppercase"><u>&nbsp;&nbsp;{{ $user->personalInfo->getFullNameAttribute() }}&nbsp;&nbsp;</u></option>
                                @endforeach
                            </select>
                            @error('chief_rps')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="col-sm-6">
                            <select name="chief_tsd" id="chief_tsd" class="form-select select2 @error('chief_tsd') is-invalid @enderror">
                                ...
                                @foreach ($_user->getAllInternals() as $user)
                                    <option value="{{ $user->id }}" {{ $user->id == old('chief_tsd') ? 'selected' : ($user->id == $permit->chief_tsd ? 'selected' : '') }} style="text-transform: uppercase"><u>&nbsp;&nbsp;{{ $user->personalInfo->getFullNameAttribute() }}&nbsp;&nbsp;</u></option>
                                @endforeach
                            </select>
                            @error('chief_tsd')
                                <span class="invalid-feedback" role="alert">
